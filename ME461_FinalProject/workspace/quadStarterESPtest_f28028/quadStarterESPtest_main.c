...
{
    //
    // WARNING: Always ens ure you call memcpy before running any functions from
...
//        rollsetpoint = atan((fabs(pinky-bluey))/(fabs(pinkx-bluex)));

        rollDif = phi_est - rollsetpoint;
        pitchDif = theta_est - pitchsetpoint;

        tau_x = -Kp_x * (rollDif) -Kd_x * (w_x) - K_dd_x * w_x_dot - Ki_x * integralPhiError;
